...
/*                                                                    */
/* Notes: 1) The function prototypes were added by Jason Fricot       */
/*           on 3/26/98 to comply with the newer C compilers.         */
/*        2) Added "#include <stdlib.h>" to eliminate warning on      */
/*           undefined function exit()                                */
/*        3) Changed "i=save_name()" to "save_name()" in function     */
/*           smpl() to eliminate warning on variable i is not used    */
/*                                                                    */
/**********************************************************************/
#include "smpl.h"
#include <stdlib.h>
...
#define timesQueueLength 65536 /*msh*/
...
static int
  queuedTimesQueueIdx,
  dequeuedTimesQueueIdx,
  numReleased;
static real
  queuedTimes[timesQueueLength],
  dequeuedTimes[timesQueueLength],
  Wq;
...
void smpl(int m, char *s)
    {
      ...
      event=0;                 /* current event no. & trace flags  */
	  tr=1;
      for (i=0; i<nl; i++)  {l1[i]=l2[i]=l3[i]=0; l4[i]=l5[i]=0.0;}
	  //msh
	  queuedTimesQueueIdx=0;
	  dequeuedTimesQueueIdx=0;
	  numReleased=0;
	  for (i=0; i<timesQueueLength; i++)  {queuedTimes[i]=dequeuedTimes[i]=0.0;}

      save_name(s,50);                   /* model name -> namespace */
...
void schedule(int ev,real te,int tkn)
    {
	  ...
	  //printf("schedule i=%d, l3[i]=%d, l5[i]=%.4f\n", i, ev, l5[i]);
    }
...
static void msg(int n,int i,char *s,int q1,int q2)
    {
      ...
      if (clock>tl)     /* print time stamp (if time has advanced) */
        then {
		  tl=clock; //fprintf(opf,"  time %-12.3f  ",clock);
		  if(n==7 && q1==1){
			queuedTimes[queuedTimesQueueIdx++] = clock;
		  }else if(n==11){
			dequeuedTimes[dequeuedTimesQueueIdx++] = clock;
		  }
		}
...
void report(void)
  {
	...
	//msh
    /*
	for(int i=0; i<queuedTimesQueueIdx; i++){
	  printf("Queued Times: %.4f\n", queuedTimes[i]);
    }
    for(int i=0; i<dequeuedTimesQueueIdx; i++){
	  printf("Dequeued Times: %.4f\n", dequeuedTimes[i]);
    }
	*/

    real summedDelays = 0.0;	

	for(int i=0; i<queuedTimesQueueIdx; i++){
	  if(i<dequeuedTimesQueueIdx)
		summedDelays += dequeuedTimes[i] - queuedTimes[i];
	  else{
		summedDelays += dequeuedTimes[dequeuedTimesQueueIdx-1] - queuedTimes[i];
		//printf("KEEP DELAYED: %.3f += %.3f - %.3f\n", summedDelays, dequeuedTimes[dequeuedTimesQueueIdx-1], queuedTimes[i]);
	  }
	}
	
	Wq = summedDelays/numReleased;
